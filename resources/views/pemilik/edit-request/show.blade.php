...
@section('title', 'Detail Edit Request')

@section('content')
<a href="{{ route('admin.edit-requests.index') }}" class="inline-flex items-center text-gray-600 hover:text-primary mb-6">
    <i class="fas fa-arrow-left mr-2"></i> Kembali
</a>

<div class="bg-white rounded-xl shadow-xl overflow-hidden">
    <div class="bg-gradient-to-r from-primary to-blue-400 px-8 py-6">
        <h1 class="text-3xl font-bold text-white">Detail Edit Request #{{ $editRequest->id }}</h1>
    </div>
    ...
    <div class="p-8">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            ...
            <div>
                <label class="block text-sm font-semibold text-gray-600 mb-1">Destinasi</label>
                <p class="text-lg font-bold">{{ $editRequest->destinasi->nama_destinasi }}</p>
                <p class="text-sm text-gray-500">{{ $editRequest->destinasi->kategori->nama_kategori ?? '-' }}</p>
            </div>
            
            <div>
                <label class="block text-sm font-semibold text-gray-600 mb-1">Tipe Request</label>
                <p class="text-lg">{{ ucfirst(str_replace('_', ' ', $editRequest->request_type)) }}</p>
            </div>
        ...
        <div class="mb-6">
            <label class="block text-sm font-semibold text-gray-600 mb-2">Keterangan dari User</label>
        ...
        <div class="mb-6">
            <label class="block text-sm font-semibold text-gray-600 mb-2">Data Request</label>
            <div class="bg-gray-50 p-4 rounded-lg">
                <pre class="text-sm text-gray-700 whitespace-pre-wrap">{{ json_encode($editRequest->request_data, JSON_PRETTY_PRINT) }}</pre>
            </div>
        </div>
        @endif
            ...
            <div class="bg-yellow-50 p-4 rounded-lg border-l-4 border-yellow-400">
                ...
                @if($editRequest->approvedBy)
                    <p class="text-sm text-gray-500 mt-2">
                        oleh: {{ $editRequest->approvedBy->name }} pada {{ $editRequest->approved_at->format('d M Y H:i') }}
                    </p>
                @endif
            </div>
        </div>
        ...
        <div class="flex gap-4">
            <form method="POST" action="{{ route('admin.edit-requests.approve', $editRequest->id) }}" class="flex-1">
                @csrf
                <textarea name="admin_notes" placeholder="Catatan (opsional)..." 
                          class="w-full px-4 py-3 border-2 rounded-lg mb-3"></textarea>
                <button type="submit" class="w-full bg-green-500 hover:bg-green-600 text-white px-6 py-3 rounded-lg font-semibold">
            ...
            <form method="POST" action="{{ route('admin.edit-requests.reject', $editRequest->id) }}" class="flex-1">
                @csrf
                <textarea name="admin_notes" placeholder="Alasan reject (wajib)..." required 
                          class="w-full px-4 py-3 border-2 rounded-lg mb-3"></textarea>
