            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2">
                    <label for="foto" class="block text-gray-700 font-semibold mb-2">
                        Foto Destinasi (Maks 3 Foto)
                    </label>
                    <div class="flex items-center gap-4">
                        <label for="foto" class="cursor-pointer bg-gray-100 hover:bg-gray-200 px-6 py-3 rounded-lg border-2 border-dashed border-gray-300 hover:border-primary transition flex items-center gap-2">
                    ...
                    @error('foto')
                        <p class="text-red-500 text-sm mt-2">{{ $message }}</p>
                    @enderror
                    <p class="text-xs text-gray-500 mt-2">Format: JPG, PNG. Maksimal 3 foto</p>
                </div>
            </div>
...
function previewImages(input) {
    ...
    Array.from(input.files).forEach(file => {
        ...
        reader.onload = function(e) {
            preview.innerHTML += `<img src="${e.target.result}" class="w-full h-40 object-cover rounded-lg shadow">`;
        }
